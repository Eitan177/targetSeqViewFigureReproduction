formatPlot <-
    function(dftailA,mismatchcolor='',title='Alignment Plot',covMaxDisplay=Inf){
        ...
        if(max(dftailA$ypostail) > covMaxDisplay){
            dfm <- list()
            dfl=split(dftailA,dftailA$ref)
            for(ii in seq_along(dfl)){
                dfm[[ii]] <- dfl[[ii]]
                allreadpos <- dfm[[ii]]$posvec2[dfm[[ii]]$ypostail != -1 & dfm[[ii]]$ypostail <= covMaxDisplay]
                dfm[[ii]] <- dfm[[ii]][-which(dfm[[ii]]$ypostail > covMaxDisplay | (dfm[[ii]]$ypostail == -1 &
                                      (dfm[[ii]]$posvec2 > max(allreadpos) | dfm[[ii]]$posvec2 < min(allreadpos)))),]
            }
            dftailA <- do.call(rbind,dfm)
        }
