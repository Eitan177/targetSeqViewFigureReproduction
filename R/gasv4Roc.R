...
function(getTop=FALSE){
    ...
    for(ii in 1:5){
        ...
        subjectreturns[[ii]]=IRanges::lapply(sovs,function(x){ss=split(subjectHits(x),queryHits(x));firsts=IRanges(start=ss[[1]]-(ss[[1]]-1)%%2,width=2);
                                                     if(length(ss)==1){print('gasv junctions not found');seconds=firsts}else{
    ...
    ## we added putative V(D)J
    tprep=c(as.numeric(elementMetadata(gasvrngs[elementMetadata(gasvrngs)[,1]=='V'])[,2]),
    as.numeric(elementMetadata(gasvrngs[elementMetadata(gasvrngs)[,1]=='P'])[,2]))

    fprep=as.numeric(elementMetadata(gasvrngs[elementMetadata(gasvrngs)[,1]=='N'])[,2])
    tprep=tprep[seq(1,length(tprep),2)]
    fprep=fprep[seq(1,length(fprep),2)]
    ## gasv does not call two of the fp events and 1 tp!!
    fprep=c(fprep)
    tprep=c(tprep)
    hp=prediction(c(tprep,fprep),c(rep(1,length(tprep)),rep(0,length(fprep))))
    hprag=performance(hp,"tpr", "fpr")
