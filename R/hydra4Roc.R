...
function(getTop=FALSE){
    ...
    for(ii in 1:5){
        ...
        hydranames=seqnames(hydrarngs[subjectHits(ovs)])
        keepovs=which(as.character(rngnames)==as.character(hydranames))
        ovs=ovs[keepovs]
...
subjectreturns[[ii]] <- subjectreturns[[ii]][as.numeric(gsub('.+\\.','',names(subjectreturns[[ii]])))>1]
    ...
    hydratprep=c(as.numeric(elementMetadata(hydrarngs[elementMetadata(hydrarngs)[,1]=='V'])[,2]),
    as.numeric(elementMetadata(hydrarngs[elementMetadata(hydrarngs)[,1]=='P'])[,2]))
    hydrafprep=as.numeric(elementMetadata(hydrarngs[elementMetadata(hydrarngs)[,1]=='N'])[,2])
    hydratprep=hydratprep[seq(1,length(hydratprep),2)]
    hydrafprep=hydrafprep[seq(1,length(hydrafprep),2)]
    hydrahp=prediction(c(hydratprep,hydrafprep),c(rep(1,length(hydratprep)),rep(0,length(hydrafprep))))
    hydrahprag=performance(hydrahp,"tpr", "fpr")
